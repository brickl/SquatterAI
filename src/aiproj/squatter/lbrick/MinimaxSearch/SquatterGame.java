...
/* SquatterGame implements the Game interface required by the AlphaBetaSearch
engine */
public class SquatterGame implements Piece, Game<Board,Move,Integer> {

    public static final int CORNERDEDUCTION = 20;
    public static final int WINWEIGHT = 100000;
    public static final int STRINGWEIGHT = 10;
    public static final int WHITEWEIGHT = 200;
    public static final int BLACKWEIGHT = 200;

    /**
     * Checks whether the game board is in a terminal state (i.e. if the game
     * is finished).
    ...
    /**
     * Finds all possible moves given the current board state, and returns them
     * in order of how likely each move is to
    ...
    public Move[] getActions(Board b){
        ...
        // Comparator require for TreeMap
        MoveComparator bvc = new MoveComparator(map);
        // Treemap used to store sorted values - makes sorting more efficient
    ...
    private int calcValue(Board b, Move m){
        ...
        //we want to look at all diagonally adjacent cells

        try{
            //diagonal to the upper right
            if(b.gameBoard[m.Row-1][m.Col+1] == currentPlayer ){
                calculatedValue +=1;
            }
    ...
    /**
     * Checks the utility weighting of the corners of the board, as being close
     *  to the corners but not in the corners
    ...
    /***
     * Assigns values to having pieces in "strings" - three nodes next to each
     * other
        ...
        for(row=0; row<b.getSize(); row++) {
            for(col=0; col<b.getSize(); col++) {
                if(b.gameBoard[row][col] == p) {
                    if ((pieceAbove(b, row, col, p)
                            && pieceBelow(b, row, col, p))
                            || (pieceToLeft(b, row, col, p)
                            && pieceToRight(b, row, col, p))) {
    ...
    /**
     * All piece_____ functions work similarly:
     * @param b Board configuration
     * @param row row to check
     * @param col column to check
     * @param p piece to look for
     * @return true if there is a piece in one of the adjacent cells above
     * (row, col)
     */
